  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        {result.signal !== "No Idea" && <>
        <div className="space-y-1">
          <p className="text-red-900">Short Position</p>
          <p className="text-xl font-bold text-red-600">{result.smaShort.toFixed(2)}</p>
        </div>
        <div className="space-y-1">
          <p className="text-green-900">Long Position</p>
          <p className="text-xl font-bold text-green-600">{result.smaLong.toFixed(2)}</p>
        </div></>}
      </div>
